...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    for (auto model:this->world->Models()){
        if (model->GetName()+"::link"==_parent->ParentName()){
            parent_model = model;
            break;
        }
    }
    std::cout<<parentEntity->GetName()<<std::endl;
    std::cout<<parentEntity->WorldPose()<<std::endl;
    std::cout<<parent_model->GetName()<<std::endl;
    std::cout<<parent_model->WorldPose()<<std::endl;
    std::cout<<raySensor->Pose()<<std::endl;
    ...
    if (rayShape) {
        ...
        for (auto &pair : points_pair) {
            ...
            //   auto index = (verticalRayCount - verticle_index - 1) * rayCount + horizon_index;
                ...
                axis = parentEntity->WorldPose().Rot().RotateVector(axis);
                axis = raySensor->Pose().Rot().RotateVector(axis);
                //auto point = range * axis;
                auto point = range * axis + parentEntity->WorldPose().Pos();//转换成世界坐标系
