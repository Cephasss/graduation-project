...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        ignition::math::Pose3d world_pose = parentEntity->WorldPose();
        ignition::math::Vector3d xyz = world_pose.Pos();
        ignition::math::Quaterniond rotate = world_pose.Rot();
        ignition::math::Vector3d rotate_euler = rotate.Euler();
        // std::cout<< rotate <<std::endl;
        xyz.Set(-xyz.X(), -xyz.Y(), xyz.Z());
        rotate_euler.Set(-rotate_euler.X(), -rotate_euler.Y(), rotate_euler.Z());
        rotate.Euler(rotate_euler);
        ...
        for (auto &pair : points_pair) {
            ...
            //   auto index = (verticalRayCount - verticle_index - 1) * rayCount + horizon_index;
                ...
                ignition::math::Quaterniond ray;
                ray.Euler(ignition::math::Vector3d(0.0, rotate_info.zenith, rotate_info.azimuth));
                auto axis = (rotate * ray).RotateVector(ignition::math::Vector3d(1.0, 0.0, 0.0));
                std::cout<< range * axis <<std::endl;
                auto point = range * axis + xyz;//转换成世界坐标系
