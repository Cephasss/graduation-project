        ...
        while(1){
          for (auto s:*solution){
            ...
            while (1){
              ...
              && abs(border_points.at(rand_index).Y()) >=0.8 * max_y
              ...
              && border_points.at(rand_index).Y() <= 0.7 * max_y && border_points.at(rand_index).Y() >= -0.7 * max_y
              ) break;
            }
              ...
              if ((int)type.Z()){
                ...
                if(border_points.at(rand_index).Z()>=max_z*0.9){
                  if(border_points.at(rand_index).Y()>=0.6*max_y){
                    if (border_points.at(rand_index).Y()>=0.8*max_y){
                ...
                if (border_points.at(rand_index).Y()>0){
                  if (border_points.at(rand_index).Y()<=0.8*max_y){
                    rot_vec.Z() = M_PI/3 * rand()/RAND_MAX  + M_PI/3*border_points.at(rand_index).Y()/max_y;
                  }else{
                    rot_vec.Z() = M_PI/2 * rand()/RAND_MAX  + M_PI/3*border_points.at(rand_index).Y()/max_y;
                  }
                }else{
                  if (border_points.at(rand_index).Y()>=-0.8*max_y){
                    rot_vec.Z() = -M_PI/3 * rand()/RAND_MAX  + M_PI/3*border_points.at(rand_index).Y()/max_y;
      ...
      while (turn <= MAX_TURN){
        ...
        for (int i=0; i<SOLUTION_NUM; i++){
          ...
          for (int sz = 0; sz<=12; sz++){
            ...
            for (int sr = 1; sr<=45; sr++){
              // if (sr>=30 && sr<=50) continue;
              for (int stheta = -60; stheta<60; stheta++){
                rot_vec.X() = 0;
                rot_vec.X() = 0;
                rot_vec.Z() = (double)stheta/24.0;
                ...
                for (auto s:*solution){
                  ...
                  if (s->type=="camera"){
                    ...
                    if (multi_count){
                      continue;
                      score_solutions[i]-=power(2,multi_count);
                    }else{
...
bool check_angle(std::vector<s_info*>* solution, int num_sen){
  ...
  for (int i=1; i<index; i++){
    // std::cout<<ranked_yaw_list[i]-ranked_yaw_list[i-1]<<std::endl;
    if (ranked_yaw_list[i]-ranked_yaw_list[i-1] < M_PI/1.1/(double)num_sen
    || ranked_yaw_list[i]-ranked_yaw_list[i-1] > M_PI/3) return false;    // Make sure all the sensors will have the seperate yaw to ensure cover ration
  }
