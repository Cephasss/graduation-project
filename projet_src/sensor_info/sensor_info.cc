      ...
      for (int i=0; i<SOLUTION_NUM; i++){
        ...
        for (auto s:*solution){
          ...
          while (1){
            ...
            if (border_points.at(rand_index).Z()>=max_z*0.9 && border_points.at(rand_index).X()>=max_x*0.34
            || border_points.at(rand_index).Z()>=max_z*0.55 && border_points.at(rand_index).X()>=max_x*0.6
            || border_points.at(rand_index).Z()>=max_z*0.3 && border_points.at(rand_index).X()>=max_x*0.95
            && border_points.at(rand_index).Y() <= 0.7 * max_y && border_points.at(rand_index).Y() >= 0.8 * max_y
            ) break;
          }
            ...
            if (type.Z()){
              if(border_points.at(rand_index).Z()>=max_z*0.9){
                if(border_points.at(rand_index).Y()>=0.6*max_y){
                  rot_vec.X() = 0;
                  rot_vec.Y() = -M_PI/6 * rand()/RAND_MAX;
                  rot_vec.Z() = M_PI/2 * rand()/RAND_MAX;
                }else if(border_points.at(rand_index).Y()<=-0.6*max_y){
                  rot_vec.X() = 0;
                  rot_vec.Y() = -M_PI/6 * rand()/RAND_MAX;
                  rot_vec.Z() = -M_PI/2 * rand()/RAND_MAX;
                }else{
                  rot_vec.X() = 0;
                  rot_vec.Y() = -M_PI/6 * rand()/RAND_MAX;
                  rot_vec.Z() = M_PI/18 * rand()/RAND_MAX + M_PI/6 * border_points.at(rand_index).Y()/max_y;
                }
              }else{
              ...
              rot_vec.X() = 0;
              rot_vec.Y() = M_PI/6 * rand()/RAND_MAX - M_PI/12;
              rot_vec.Z() = M_PI/3*2 * rand()/RAND_MAX - M_PI/3;
            }else if(type.Y()==1){
              if (border_points.at(rand_index).Y()<=0.9*max_y){
                rot_vec.X() = 0;
                rot_vec.Y() = M_PI/6 * rand()/RAND_MAX - M_PI/12;
                rot_vec.Z() = M_PI/18 * rand()/RAND_MAX + M_PI/6 * border_points.at(rand_index).Y()/max_y;
              }else{
                rot_vec.X() = 0;
                rot_vec.Y() = M_PI/6 * rand()/RAND_MAX - M_PI/12;
                rot_vec.Z() = M_PI*0.6 * rand()/RAND_MAX + M_PI/6;
              }
            }else if(type.Y()==-1){
              if (border_points.at(rand_index).Y()>=0.9*-max_y){
                rot_vec.X() = 0;
      ...
      while (turn <= MAX_TURN){
        ...
        for (int i=0; i<SOLUTION_NUM; i++){
          ...
          for (int sz = 1; sz<12; sz++){
            ...
            for (int sy = -36; sy<36; sy++){
              for (int sx = -32; sx<60; sx++){
                // Skip if too close or inside the car
                if (sz/5<max_z && sx/4>min_x && sx/4<max_x && sy/4>min_y && sy/4<max_y) continue;
          ...
          //         sVec.X() = sx+(double)bias/5;
          //         sVec.Y() = sy+(double)bias/5;
          //         sVec.Z() = sz+(double)bias/5;
          //         for (auto s:*solution){
          //           sVec -= s->model_pose.Pos();
          //           if (sVec.Length()<s->near || sVec.Length()>s->far) continue;
          //           if (s->type=="camera"){
          ...
          //             if (sVec.Dot(s->nor3)<=0) continue;
          //             if (sVec.Dot(s->nor4)<=0) continue;
          //             score_solutions[i]-=100;
          //           }else if(s->type=="ray"){
          //             if (acos(sVec.Dot(s->nor1)/s->nor1.Length()) > s->lidar_fov.Radian()) continue;
          //             score_solutions[i]-=100;
          //           }
          ...
          //   }
          // }
          cover_ratio = cover_ratio / (12*61*81);
          for (auto s1:*solution){
            for (auto s2:*solution){
              if(s1->model_name==s2->model_name) continue;
              score_solutions[i]-=1/(s1->model_pose.Pos()-s2->model_pose.Pos()).Length();
            }
          }
        ...
        if (last_highest_score!=-500000){
          if (abs(last_highest_score-score_solutions[0])<1){
            unchanged_turns++;
            MUT_PROBA+=(1-MUT_PROBA)*unchanged_turns*0.05;
            CRO_PROBA+=(1-CRO_PROBA)*unchanged_turns*0.05;
            if (MUT_PROBA>=1) MUT_PROBA=1;
            if (CRO_PROBA>=1) CRO_PROBA=1;
          }else if(last_highest_score<score_solutions[0]-1.1){
            MUT_PROBA = MUT_PROBA*0.4;
